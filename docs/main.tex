...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[node distance=0.95cm and 1.7cm]

    % ── Cadena de llegada (fila superior, 3 nodos) ──────────────────────
    ...
    % ── Fase de carga (centrado bajo la cola) ───────────────────────────
    \node[resourcebox, below=2.0cm of cola] (carga)
      {Carga de embarcación\\$\geq$24\,tur.\ o 15\,min};

    % ── Ciclo operativo ─────────────────────────────────────────────────
    \node[flowbox, below=1.6cm of carga] (ciclo)
      {Ciclo operativo\\nav.\ ida\,$\cdot$\,permanencia\\retorno\,$\cdot$\,preparación};

    % ── Métricas (panel derecho) ─────────────────────────────────────────
    \node[outputbox, right=2.0cm of carga] (metricas)
      {Métricas del sistema\\espera\,$\cdot$\,$L_q$\,$\cdot$\,utilización\\
       turistas atendidos / no atendidos};
    ...
    % ── Arco de retorno (izquierda, curva suave) ─────────────────────────
    \draw[dashedarrow] (ciclo.west)
      to[out=180, in=180, looseness=1.15]
      node[left, font=\scriptsize\itshape, pos=0.55] {disponible}
      (cola.west);

    % ── Métricas ─────────────────────────────────────────────────────────
    \draw[dashedarrow] (carga.east) -- (metricas.west);
    \draw[dashedarrow] (ciclo.east) -- ++(1.0,0)
      |- (metricas.south);

    % ── Recuadro de fondo ────────────────────────────────────────────────
    \begin{scope}[on background layer]
      \node[draw=black!18, rounded corners=6pt, fill=blue!2,
        fit=(llegada)(registro)(cola)(carga)(ciclo)(metricas),
        inner sep=0.4cm] {};
    \end{scope}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[node distance=1.1cm and 1.8cm]
    ...
    % ── Fila superior: tres pilares del modelo ───────────────────────────
    \node[flowbox] (entradas)
      {Entradas aleatorias\\interllegadas\,$\cdot$\,tamaño\\
       registro\,$\cdot$\,navegación\,$\cdot$\,permanencia};
    \node[resourcebox, right=of entradas] (modelo)
      {Modelo Montecarlo\\FIFO\,$\cdot$\,capacidad\,$\cdot$\,umbral\\
       1\,000 réplicas por escenario};
    \node[outputbox, right=of modelo] (resultados)
      {Resultados por jornada\\espera\,$\cdot$\,$L_q$\,$\cdot$\,utilización\\
       turistas atendidos / no atendidos};

    % ── Segunda fila: comparación ────────────────────────────────────────
    \node[decisionbox, below=2.0cm of modelo] (comparacion)
      {Comparación\\de escenarios\\E0\,--\,E4};

    % ── Tercera fila: recomendación ──────────────────────────────────────
    \node[outputbox, below=1.8cm of comparacion] (recomendacion)
      {Recomendación operativa\\E1 (más embarcaciones)\\o E3 (mayor capacidad)};

    % ── Flechas ──────────────────────────────────────────────────────────
    \draw[flowarrow] (entradas)   -- (modelo);
    \draw[flowarrow] (modelo)     -- (resultados);
    \draw[flowarrow] (resultados.south) -- ++(0,-0.6)
