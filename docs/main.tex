...
de Puno entre las 06:00 y las 10:00. El flujo es:

\begin{center}
\small
Llegada grupo $\;\to\;$ Cola ventanilla $\;\to\;$ Registro/pago $\;\to\;$
Cola embarque (FIFO) $\;\to\;$ Asignación a embarcación $\;\to\;$
Viaje ida $\;\to\;$ Permanencia $\;\to\;$ Retorno $\;\to\;$ Preparación
\end{center}
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}clrrrll@{}}
    \toprule
    N\textsuperscript{o} & Hora llegada & Interllegada & Tamaño grupo & Registro & Destino & Observación \\
     & & (min) & (turistas) & (min) & & \\
    \midrule
...
\begin{table}[H]
  ...
  \small
  \begin{tabular}{@{}llll p{4.5cm}@{}}
    \toprule
    Variable & Símbolo & Distribución & Parámetros & Justificación \\
    \midrule
    Tiempo entre llegadas & $T_a$ & Exponencial & $\mu = 7{,}5$ min & Llegadas
      independientes; 8 grupos/hora.\\[4pt]
    Tamaño de grupo & $G$ & Triangular discreta & min~4, moda~10, max~18 &
      Ficha de observación sintética.\\[4pt]
    Tiempo de registro & $T_r$ & Triangular & min~3, moda~5, max~7 &
      Proceso manual con variabilidad moderada.\\[4pt]
    Navegación ida & $T_{n1}$ & Uniforme & min~25, max~40 &
      Sin preferencia clara; depende del viento (PDF base).\\[4pt]
    Permanencia en isla & $T_p$ & Triangular & min~45, moda~90, max~180 &
      Existe valor más probable, con amplia variabilidad (PDF base).\\[4pt]
    Navegación retorno & $T_{n2}$ & Uniforme & min~25, max~40 &
      Supuesto simétrico con la ida.\\[4pt]
    Preparación & $T_{prep}$ & Constante & 10 min & Regla operativa fija
      (PDF base).\\[4pt]
    Capacidad embarcación & $C$ & Constante & 30 turistas & Regla base
      (PDF base).\\[4pt]
    N\textsuperscript{o} embarcaciones & $c$ & Constante & 3 & Regla base
      (PDF base).\\
    \bottomrule
  \end{tabular}
\end{table}
